                    ...
                    async with session.get(commit_url, headers={"Authorization": f"Bearer {gh_apikey}"}) as response:
                        if response.status == 200:
                            ...
                            try : 
                                ...
                                asyncio.create_task(append_to_json(new_data,'files_3_.json'))
                                return "OK"
                            except : 
                                new_data = {
                                'keyword':keyword,
...
def save_to_json(data, filename="_commits_results_part3.json"):
    try:
...
def save_to_json(data, filename="commits_results.json"):
    try:
        ...
        print(f"Error al guardar en archivo JSON: {e}")
async def main_():
    #data_ = await load_save_data('files_3.json')
    data_ = await load_save_data('empty_files_3.json')
    ...
    #save_to_json(load_data,"empty_files_3.json")
    #return
    print(f'{len(load_data)} datos cargados....')
    tasks = []
    async with aiohttp.ClientSession() as session:
        with open('_commits_results_part3.json','r+') as file : 
            data = json.load(file)
            print(len(data))
