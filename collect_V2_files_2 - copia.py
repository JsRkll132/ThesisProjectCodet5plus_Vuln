...
async def main_():
    data_ = await load_save_data('AppendData/append_files_4.json')
    data_sha = [d['sha'] for d in data_]
    print(f'{len(data_)} _cleaned_')
    visited_sha = [ ]
    visited = [ ]
    for i in data_ :
        print(i['sha'])
        if i['sha'] not in visited : 
            visited_sha.append(i['sha'])
            visited.append(i)
        else : 
            pass #print(f'{ i['sha'] } visited ..')
    print(f'{len(visited)} not double ')
    """
    clean_original_data = [ i for i in data_ if len(i['files']) != 0 ]
    new_data = await load_save_data('files_4_.json')
    print(f'{len(data_)} not_cleaned_')
    print(f'{len(clean_original_data)} _cleaned_')
    print(f'{len(new_data)} _new_')
    clean_original_data.extend(new_data)
    save_to_json(clean_original_data,'AppendData/append_files_4.json')"""
    #load_data = [file['files'] for file in data_ if len(file) != 0 ]
   ...
   # print(f'{len(load_data)} datos cargados...')
    ...
    tasks = []
    async with aiohttp.ClientSession() as session:
        with open('files_3_.json','r+') as file : 
            data = json.load(file)
            print(len(data))
