...
gh_apikey = os.getenv('TOKEN_GITHUB_3')
github_tokens = [
                    ...
                    async with session.get(commit_url, headers={"Authorization": f"Bearer {gh_apikey}"}) as response:
                        if response.status == 200:
                            ...
                            try : 
                                ...
                                asyncio.create_task(append_to_json(new_data,'files_4_.json'))
...
async def main_():
    ...
    data_ = await load_save_data('empty_files_4.json')
    ...
    #save_to_json(load_data,"empty_files_4.json")
    #return
    tasks = []
    async with aiohttp.ClientSession() as session:
        with open('_commits_results_part4.json','r+') as file : 
            data = json.load(file)
            print(len(data))
