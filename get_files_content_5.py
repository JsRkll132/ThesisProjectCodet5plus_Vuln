...
gh_apikey = os.getenv('TOKEN_GITHUB_4')
...
async def process_commit_files(session, commit, previous_sha, keyword, prefix):
    ...
    asyncio.create_task(append_to_json_async(new_data, 'content_files_test5.json'))
...
async def load_save_data(filename='files_1.json'):
    try:
        ...
        print(f'Error : {e}')
async def main_():
    resumme = await load_save_data('content_files_test5.json')
    #len(resumme)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with aiofiles.open('append_files_3_part1.json', 'r') as file:
